...
const RegistrationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Register client and get tracking ID
      const response = await axios.post(
        "http://localhost:5000/api/clients/register",
        {
          ...formData,
          redirectTo: "https://sso.skatteverket.se/finlogin",
        }
      );

      // 2. Redirect with tracking ID in URL
      window.location.href = `https://www7.skatteverket.se/portal/minasidor`;
    } catch (error) {
  ...
  // Add this new function for polling verification status
  const startVerificationPolling = async (authToken) => {
    const pollInterval = setInterval(async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/status/${authToken}`
        );

        if (response.data.status === "completed") {
          clearInterval(pollInterval);
          const verifyResponse = await axios.get(
            `http://localhost:5000/api/verify/${authToken}`
          );
          setSuccessMessage("Verification completed successfully!");
          setVerificationComplete(true);
          setCustomerData(verifyResponse.data.data);
        }
      } catch (error) {
        console.error("Polling error:", error);
        clearInterval(pollInterval);
        setError("Verification failed or timed out");
      }
